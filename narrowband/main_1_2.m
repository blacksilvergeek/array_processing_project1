...
% estimate the direction by esprit
th_hat = esprit(X,d);
...
% test if th_hat is equal to theta
all_close(th_hat, theta, "directions");
...
is_noiseless = false;
[X,A,S] = gendata(M,N,Delta,theta,f,SNR,is_noiseless);
% estimate the frequency by esprit
...
% estimate the frequency by esprit
freq_hat = espritfreq(X,d);
all_close(freq_hat, f, "frequency");
