...
algo = "espritfreq";
dbs = 0:2:20;
f_esprit_averages = zeros(2, length(dbs));
f_esprit_deviations = zeros(2, length(dbs));


for k = 1:length(dbs)
    % set SNR
    SNR = dbs(k);
    results = mean_and_deviation(SNR, "espritfreq");
    f_esprit_averages(:,k) = results(:,1);
    f_esprit_deviations(:,k) = results(:,2);
    
end


% plot the average and standard deviation
errorbar(dbs, f_esprit_averages(1,:), f_esprit_deviations(1,:), 'o-', 'LineWidth', 1.5);
hold on
errorbar(dbs, f_esprit_averages(2,:), f_esprit_deviations(2,:), 'o-', 'LineWidth', 1.5);
